...
from admin_services import add_stock, broadcast_stock_update, delete_expense, delete_product, load_gpt_marks, load_materials, release_holds, release_order, save_expense, save_gpt_marks, save_materials, save_product, save_promotion, save_promo_settings, save_slot, snapshot, update_order, update_stock_item
...
def register_admin_routes(app: FastAPI) -> None:
    ...
    async def admin_delete_product(product_id: str, request: Request):
        require_admin(request)
        try:
            return await asyncio.to_thread(delete_product, product_id)
        except Exception as exc:
            raise HTTPException(status_code=500, detail=str(exc)) from exc

    @app.post("/admin/api/products/broadcast-stock")
    async def admin_broadcast_stock(request: Request):
        require_admin(request)
        return await asyncio.to_thread(broadcast_stock_update)
    ...
    @app.get("/admin/api/materials")
    async def admin_get_materials(request: Request):
        require_admin(request)
        try:
    ...
    @app.post("/admin/api/promo-settings")
    async def admin_save_promo_settings(request: Request):
        require_admin(request)
        try:
    ...
    async def admin_save_gpt_marks(request: Request):
        require_admin(request)
        try:
            return await asyncio.to_thread(save_gpt_marks, await request.json())
        except Exception as exc:
            raise HTTPException(status_code=500, detail=str(exc)) from exc

    @app.get("/admin/api/gpt-marks")
    async def admin_get_gpt_marks(request: Request):
        require_admin(request)
        try:
            items = await asyncio.to_thread(load_gpt_marks)
            return {"ok": True, "items": items}
        except Exception as exc:
            logger.exception("admin_get_gpt_marks failed")
            return {"ok": False, "error": str(exc), "source": "gpt_marks"}
